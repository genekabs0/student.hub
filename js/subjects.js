...
async function deleteSubject() {
    ...
    // Soft delete related data (mark as deleted with timestamp)
    const assignments = await db.assignments.where('subjectId').equals(currentSubject.id).toArray();
    ...
    const files = await db.files.where('subjectId').equals(currentSubject.id).toArray();
    for (const f of files) {
        await db.files.update(f.id, { deleted: true, lastModified: now });
    }
...
function initSubjects() {
    ...
    // Folder button - navigate to storage with subject filter
    document.getElementById('subject-folder-btn').addEventListener('click', () => {
        if (currentSubject) {
            // Store current subject ID for storage page
            Storage.set('selectedSubjectForStorage', currentSubject.id);
            // Navigate to storage page
            document.querySelectorAll('.nav-link').forEach(link => link.classList.remove('active'));
            document.querySelector('[data-page="storage"]').classList.add('active');
            document.querySelectorAll('.page').forEach(page => page.classList.remove('active'));
            document.getElementById('storage-page').classList.add('active');
            document.querySelector('.page-title').textContent = 'Storage';
            // Render storage for this subject
