...
async function renderAssignments() {
    ...
    const nonDeletedAssignments = allAssignments.filter(a => !a.deleted);

    // Filter assignments
    let assignments = nonDeletedAssignments;
    if (currentFilter === 'completed') {
        assignments = nonDeletedAssignments.filter(a => a.completed && !a.archived);
    } else if (currentFilter === 'incomplete') {
        assignments = nonDeletedAssignments.filter(a => !a.completed && !a.archived);
    } else if (currentFilter === 'archived') {
        assignments = nonDeletedAssignments.filter(a => a.archived);
